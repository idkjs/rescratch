...
[@bs.obj]
external options:
  (~cwd: string=?, ~encoding: string=?, ~shell: bool=?, unit) => options;
...
[@bs.send.pipe: t]
external onClose:
  ([@bs.as "close"] _, (~code: int, ~signal: string) => unit) => unit =
  "on";
...
[@bs.send.pipe: t]
external onExit:
  ([@bs.as "exit"] _, (~code: int, ~signal: string) => unit) => unit =
  "on";
...
[@bs.get] external stderr: t => ReadableStream.t;

[@bs.get] external stdout: t => ReadableStream.t;